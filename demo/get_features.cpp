...
void getSift(const cv::Ptr<cv::xfeatures2d::SIFT>& sift, const cv::Mat& image, vector<cv::Mat>& features) {
  // 1600 2276
  cv::Mat dst;
  cv::resize(image, dst, cv::Size(1024, 1600), 0, 0);
  std::vector<cv::KeyPoint> keypoints;
  sift->detect( dst, keypoints );

  cv::Mat descriptors;
  sift->compute( dst, keypoints, descriptors );
