...
void testVocCreation(const vector<vector<cv::Mat > > &features, const string& saveto)
{
  ...
  vector<vector<vector<float>>> sift_features(features.size());
  for (int i = 0; i < features.size(); ++i) {
    sift_features[i].resize(features[i].size());
    ...
    for (int j = 0; j < features[i].size(); ++j) {
      ...
      sift_features[i][j].resize(features[i][j].cols);
      for (int k = 0; k < features[i][j].cols; k++) {
        sift_features[i][j][k] = features[i][j].at<float>(0, k);
      }
      cout << "copied descriptor " << i << ',' << j << " size " << sift_features[i][j].size() << endl;
    }
  }
  cout << "Creating a small " << k << "^" << L << " vocabulary..." << endl;
  voc.create(sift_features);
  //voc.create(features);
